...
A geração Raptor Lake da Intel, introduzida em 2022, consolidou a arquitetura heterogênea que combina P-cores baseados na microarquitetura Golden Cove com E-cores baseados na microarquitetura Gracemont em um mesmo pacote processador \cite{intel2021hybrid,intel2022raptorlake}. Os P-cores operam com suporte a \textit{Hyper-Threading}, enquanto os E-cores não possuem SMT, otimizando a densidade de núcleos para tarefas de baixa prioridade e cargas paralelizáveis de menor intensidade computacional.
...
Os experimentos foram conduzidos em uma estação de trabalho equipada com processador Intel Core i5-13420H (13ª geração, microarquitetura Raptor Lake), 8~GiB de RAM DDR4 instalados (7,6~GiB úteis ao sistema), e armazenamento NVMe com controlador SK Hynix (ID PCI 1C5C:1D59). O sistema operacional hospedeiro é o Windows 11, e os testes foram executados sobre o subsistema Ubuntu 24.04.3~LTS com kernel 6.6.87.2-microsoft-standard-WSL2. A GPU dedicada identificada opera via barramento PCIe Gen~4x8 (ID PCI 10DE:28A1).
...
A ausência de queda de desempenho entre os três tamanhos evidencia que o array de 16~MiB já supera o cache L3 de 12~MiB, estabelecendo a memória principal como nível operante em todos os testes. Para observar a transição entre L3 e DRAM, seria necessário testar arrays em torno de 6--10~MiB. A vazão estável em torno de 7,3~GiB/s corresponde à largura de banda sustentada por uma única \textit{stream} sequencial de cópia --- o \texttt{mbw} opera com um único fluxo de dados (\textit{single-thread}). O teto teórico agregado do canal DDR4 é da ordem de 25,6~GB/s; atingi-lo requer múltiplos fluxos de memória concorrentes. Portanto, 7,3~GiB/s representa o limite de uma única \textit{stream} sequencial, coerente com o regime \textit{memory-bound} identificado nos demais experimentos.

O teste de localidade espacial registrou 4,54~s para o \textit{loop} com acesso sequencial por linhas da matriz e 5,17~s para o \textit{loop} com acesso por colunas, razão de 1,14x. Essa diferença indica que o acesso não-contíguo por colunas degrada a eficácia do \textit{prefetcher} de hardware e a localidade de cache e TLB: cada passo entre elementos adjacentes de uma coluna representa um salto de 32~KiB, impedindo a antecipação automática de linhas de cache contíguas. O custo dominante neste experimento é, contudo, o \textit{overhead} do interpretador Python, que percorre a matriz elemento a elemento em laço puro; o efeito de localidade permanece observável mesmo parcialmente mascarado por esse custo. \textit{TLB misses} e caminhadas na tabela de páginas (\textit{page table walks}) são um dos fatores plausíveis da degradação, mas não constituem sua causa única nem principal.
...
A inspeção do processo Firefox via \texttt{/proc/PID/status} revelou \texttt{VmPeak} de 20.409.104~kB (aproximadamente 19,5~GiB) e \texttt{VmRSS} de 573.028~kB (aproximadamente 560~MB). A discrepância de 35x entre o espaço de endereçamento virtual reservado e a memória física ocupada ilustra o mecanismo de \textit{demand paging} do Linux: o kernel mapeia páginas virtuais sem alocar frames físicos até o primeiro acesso, tornando possível que um processo reserve dezenas de gigabytes de espaço virtual em uma máquina com apenas 8~GiB de RAM instalados (7,6~GiB úteis ao sistema) \cite{tanenbaum2015}.
...
O controlador NVMe opera em Gen~3x4 apesar de possuir capacidade para Gen~4x4, limitando a largura de banda bruta máxima de 8~GB/s (teórico Gen~4x4) para ~3,94~GB/s brutos (~3,5~GB/s úteis, teórico Gen~3x4). O \textit{throughput} sequencial medido pelo \texttt{fio} de 1.527~MB/s é substancialmente inferior a esse limite, o que indica que o link PCIe Gen~3x4 \emph{não} é o gargalo do I/O sequencial observado --- o fator limitante é a camada de virtualização do WSL2, que interpõe uma chamada ao hipervisor a cada operação de bloco. O resultado aponta subutilização da capacidade de interconexão do dispositivo; recomenda-se auditar a configuração do link na BIOS para verificar a possibilidade de habilitação do modo Gen~4x4.

\subsection{Interrupções e I/O}

A contagem de interrupções antes e após a transferência de 2~GB via \texttt{dd} registrou incrementos de +2.054 na linha \texttt{CAL} (\textit{Inter-Processor Interrupts}) e +2.008 na linha \texttt{HYP} (\textit{Hypervisor callback interrupts}). A correspondência entre os 2.008 \textit{callbacks} e os 2.000 blocos de 1~MiB transferidos --- constatada por medição direta neste trabalho por meio do contador \texttt{/proc/interrupts} \cite{love2010linux} --- confirma que cada conclusão de bloco gerou uma notificação separada do hipervisor Hyper-V para o kernel Linux convidado. Esse comportamento é consequência estrutural da virtualização de dispositivos no WSL2 \cite{tanenbaum2015}: o armazenamento é mediado pela camada Hyper-V, que interpõe um \textit{callback} a cada operação de I/O completada, replicando para o sistema operacional convidado a semântica de interrupção de um controlador NVMe físico. A conclusão sobre o overhead de I/O no WSL2 apoia-se, portanto, na evidência medida neste trabalho, não em fontes externas.
...
\section{Artefatos do Repositório}
\label{sec:anexo}

Os artefatos brutos que sustentam todos os resultados apresentados neste artigo encontram-se disponíveis em repositório público, garantindo a reprodutibilidade dos experimentos:

\begin{center}
\url{https://github.com/Pedro-Coutinho2612/infra-hw-artigo}
\end{center}

\noindent\textbf{Saídas de medição} (diretório \texttt{dados/}):

\begin{itemize}
  \item \texttt{info\_cpu.txt} --- saída completa do \texttt{lscpu} com identificação do processador
  \item \texttt{cinebench\_r23.txt} --- resultados do Cinebench R23 (\textit{single} e \textit{multi core})
  \item \texttt{saida\_paralelismo.txt} --- resultados do teste de escalabilidade de \textit{threads}
  \item \texttt{saida\_localidade.txt} --- resultados do teste de localidade espacial (linhas vs. colunas)
  \item \texttt{saida\_workload.txt} --- resultados do workload integrado (fases CPU, memória e I/O)
  \item \texttt{medicoes\_mbw.txt} --- saída do \texttt{mbw} para arrays de 16, 128 e 1.024~MiB
  \item \texttt{medicoes\_fio.txt} --- saída do \texttt{fio} (sequencial 1~MiB e aleatório 4~KiB)
  \item \texttt{medicoes\_memoria.txt} --- saída de \texttt{free}, \texttt{vmstat} e \texttt{/proc/PID/status}
  \item \texttt{medicoes\_pcie.txt} --- links PCIe (\texttt{LnkCap}/\texttt{LnkSta}) via \texttt{lspci -vv}
  \item \texttt{medicoes\_interrupcoes.txt} --- diferença de \texttt{/proc/interrupts} antes/após \texttt{dd}
\end{itemize}

\noindent\textbf{Scripts Python} (diretório \texttt{scripts/}):

\begin{itemize}
  \item \texttt{teste\_paralelismo.py} --- varia o número de processos (1, 2, 4, máx.) e mede \textit{speedup}
  \item \texttt{teste\_localidade.py} --- compara acesso por linhas e por colunas em matriz NumPy
  \item \texttt{workload\_completo.py} --- três fases (CPU-\textit{bound}, \textit{memory-bound}, I/O-\textit{bound}) com monitoramento por núcleo via \texttt{psutil}
\end{itemize}
