...
\begin{enumerate}
    \item \textbf{Saturação de Paralelismo}: Escalar de 1 para 4 threads reduziu o tempo de execução de 0,9804\,s para 0,6242\,s ($-36{,}33\%$). Ao saturar com 12 threads, o tempo estabilizou em 0,6414\,s, sem melhoria significativa (p\text{-}valor $= 0{,}0663 > 0{,}05$), evidenciando a barreira do barramento DDR4 em provável configuração de canal simples.

    \item \textbf{Localidade Espacial}: O percurso por colunas em uma matriz de 128\,MB gerou overhead de $4{,}11\%$ sobre o percurso por linhas (p\text{-}valor $= 0{,}0281 < 0{,}05$), confirmando penalidade sistemática associada à degradação de \textit{prefetch} e de localidade de \textit{cache}/TLB.
...
A literatura de arquiteturas \textit{multicore} estabelece que a largura de banda de memória compartilhada se torna o fator limitante do escalonamento à medida que o número de núcleos ativos cresce. \citeonline{rogers2009bandwidth} formalizaram o problema da \textit{bandwidth wall}, mostrando que a banda disponível por núcleo decresce conforme se multiplicam as unidades de processamento em um mesmo \textit{die}. \citeonline{diamond2011evaluation} demonstraram experimentalmente que, uma vez saturado o subsistema de memória, o padrão e a ordem dos acessos passam a determinar o desempenho, e que métricas tradicionais de núcleo único podem ser enganosas no regime \textit{multicore}. No domínio das arquiteturas híbridas, a documentação técnica da Intel \cite{intel2021hybrid} descreve a coexistência de P-cores e E-cores e os mecanismos de escalonamento associados, contexto em que a contenção pelo canal de memória compartilhado tende a anular ganhos esperados do paralelismo em \textit{workloads} intensivos em memória. A teoria de paginação e alocação tardia é fundamentada em \citeonline{tanenbaum2015} e \citeonline{love2010linux}.

\section{Arquitetura Híbrida Intel Raptor Lake}
\label{sec:raptor_lake}

A microarquitetura Raptor Lake \cite{intel2021hybrid,intel2022raptorlake} integra dois tipos de núcleos:
...
No i5-13420H, cada P-core possui L1d de 48\,KiB e L2 privado de 2\,MiB; os E-cores compartilham 2\,MiB de L2 por \textit{cluster}; o LLC (L3) é de 12\,MiB compartilhado. O controlador de memória integrado (IMC), em provável configuração de canal simples DDR4 estimada em 3200\,MT/s conforme a especificação da plataforma, limita a taxa teórica agregada a $\approx 25{,}6$\,GB/s, compartilhada entre todos os 12 threads.

O kernel do Linux utiliza paginação de quatro níveis (PML4 $\to$ PDPT $\to$ PD $\to$ PT) com alocação tardia (\textit{lazy allocation}): frames físicos são alocados apenas na ocorrência de \textit{page fault}. O TLB armazena traduções recentes em silício; percursos por colunas com saltos maiores que 32\,KiB tendem a reduzir o reaproveitamento de traduções e a eficácia do \textit{prefetcher}, adicionando latência mensurável.
...
\begin{itemize}
    ...
    \item RAM: 8\,GiB DDR4 instalados (7,6\,GiB úteis ao sistema), em provável configuração de canal simples estimada em 3200\,MT/s conforme especificação da plataforma.
...
Na fase CPU-bound, a taxa de 3.792,03\,Mop/s com desvio de 453,89\,Mop/s reflete o escalonamento dinâmico em ambiente virtualizado. A fase Memory-bound atingiu 7,07\,GB/s; esse valor corresponde à banda sustentada por uma única \textit{stream} de cópia sequencial (a operação \texttt{np.copyto} executa em uma única thread), e não à saturação agregada do canal. O teto teórico agregado do canal simples DDR4 ($\approx 25{,}6$\,GB/s) só seria aproximado com múltiplos fluxos concorrentes; a banda de 7,07\,GB/s de um único fluxo é, portanto, coerente com o regime \textit{memory-bound} observado. A fase I/O-bound apresentou o maior coeficiente de variação ($14\%$), com desvio de 91,23\,MB/s, diretamente associado à cadeia \textit{VM-exit}/Hyper-V/\textit{VM-enter} nas interrupções virtualizadas do subsistema de disco \texttt{.vhdx}.
...
Os ensaios comprovam que o i5-13420H, neste ambiente, é predominantemente \textit{memory-bound}. Com 4 processos matriciais paralelos, o IMC atinge o limite dinâmico de vazão DDR4. Alocar 8 threads adicionais mantém as unidades aritméticas em estado ocioso (\textit{stalled}), aguardando resolução de leituras na DRAM física — fenômeno previsto pelo modelo Roofline \cite{williams2009} e consistente com a contenção de banda compartilhada descrita em \citeonline{rogers2009bandwidth} e \citeonline{diamond2011evaluation}. O overhead de $4{,}11\%$ no percurso por colunas decorre da degradação das predições do \textit{prefetcher} e da menor localidade de \textit{cache}/TLB no acesso não-contíguo; \textit{page table walks} adicionais na DRAM constituem um dos fatores plausíveis, ainda que o efeito esteja parcialmente atenuado pelo pré-buscador do i5-13420H e mascarado pelo \textit{overhead} do interpretador. A assinatura temporal, contudo, permanece estatisticamente mensurável.
...
\begin{enumerate}
    ...
    \item \textbf{Fixar afinidade nos P-cores}: Verificar o mapeamento real dos núcleos com \texttt{lscpu -e} antes de usar \texttt{sched\_setaffinity}; em seguida, travar threads críticas nos índices lógicos correspondentes aos P-cores, impedindo desvio para os E-cores de menor desempenho vetorial. Note-se que, sob WSL2, a numeração lógica pode não seguir a ordem física esperada.
    \item \textbf{Garantir acesso sequencial por linhas}: Em loops NumPy, a travessia orientada a linhas (padrão C contíguo) minimiza \textit{cache misses} e \textit{TLB misses}, evitando a penalidade estatisticamente comprovada de $4{,}11\%$.
    \item \textbf{Auditar o link PCIe do SSD NVMe}: Verificar na BIOS se o link está negociado em Gen~4x4 (até $\approx 8$\,GB/s) em vez de Gen~3x4 (cujo limite prático é de $\approx 3{,}5$\,GB/s úteis, ou $\approx 3{,}94$\,GB/s brutos), desativando estados agressivos de economia de energia de barramento (PCIe L1) se necessário. Observa-se que o \textit{throughput} sequencial medido pelo \texttt{fio} (1.527\,MB/s) está muito abaixo do limite do link Gen~3x4, o que indica que o gargalo do I/O sequencial é a camada de virtualização do WSL2, e não a geração do enlace PCIe.
\end{enumerate}
...
\chapter{Artefatos do Repositório}
\label{anexo:artefatos}

Para fins de reprodutibilidade, todos os dados brutos coletados e os scripts utilizados nos experimentos encontram-se disponíveis em repositório público. Os artefatos estão organizados nas pastas \texttt{dados/} (medições originais) e \texttt{scripts/} (implementações em Python com protocolo estatístico).

\noindent\textbf{Arquivos de dados (\texttt{dados/}):}
\begin{itemize}
    \item \texttt{info\_cpu.txt} — saída de \texttt{lscpu} (identificação da CPU);
    \item \texttt{saida\_paralelismo.txt} — médias, IC 95\% e p-valor do teste de paralelismo;
    \item \texttt{saida\_localidade.txt} — IC 95\% e p-valor do teste de localidade;
    \item \texttt{saida\_workload.txt} — médias e IC 95\% das fases do workload integrado;
    \item \texttt{medicoes\_mbw.txt} — largura de banda de memória (\texttt{mbw});
    \item \texttt{medicoes\_fio.txt} — benchmark de armazenamento (\texttt{fio});
    \item \texttt{medicoes\_memoria.txt} — \texttt{free}, \texttt{vmstat} e \texttt{/proc/PID/status};
    \item \texttt{medicoes\_pcie.txt} — links PCIe (LnkCap e LnkSta);
    \item \texttt{medicoes\_interrupcoes.txt} — contagem de interrupções durante I/O;
    \item \texttt{cinebench\_r23.txt} — pontuações do Cinebench R23.
\end{itemize}

\noindent\textbf{Scripts (\texttt{scripts/}):}
\begin{itemize}
    \item \texttt{teste\_paralelismo.py};
    \item \texttt{teste\_localidade.py};
    \item \texttt{workload\_completo.py}.
\end{itemize}

\noindent\textbf{Repositório:} \url{https://github.com/Pedro-Coutinho2612/infra-hw-artigo}
